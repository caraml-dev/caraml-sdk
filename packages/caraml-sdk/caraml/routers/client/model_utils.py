...
def get_oneof_instance(cls, model_kwargs, constant_kwargs, model_arg=None):
    ...
    for oneof_class in cls._composed_schemas["oneOf"]:
        ...
        if not single_value_input:
            ...
            fixed_model_args = change_keys_js_to_python(model_kwargs, oneof_class)
...
def get_additional_properties_model_instances(composed_instances, self):
...
def validate_get_composed_info(constant_args, model_args, self):
    ...
    if (
        len(unused_args) > 0
        and len(additional_properties_model_instances) == 0
        and (
            self._configuration is None or not self._configuration.discard_unknown_keys
        )
    ):
        raise ApiValueError(
