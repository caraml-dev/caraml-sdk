...
class TrafficRuleCondition(ModelNormal):
    ...
    def __init__(self, field_source, field, values, *args, **kwargs):  # noqa: E501
        """TrafficRuleCondition - a model defined in OpenAPI

        Args:
            field_source (FieldSource):
            field (str): For HTTP_JSON protocol, the valid `field_source` are `header` and `payload`. Whereas, for UPI_V1 protocol the valid `field_source` are `header` and `prediction_context`. If `field_source` is `header`, then `field` should contain the name of the request header. If `field_source` is `payload`, then `field` should be a valid json path. If `field_source` is `prediction_context`, then `field` should contain variable name stored in `prediction_context` field of the incoming request.
            values ([str]):

        Keyword Args:
            operator (str): defaults to "in", must be one of ["in", ]  # noqa: E501
